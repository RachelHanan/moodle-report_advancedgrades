...
$data = $DB->get_records_sql("SELECT    grf.id AS grfid, crs.shortname AS course, asg.name AS assignment,
                                        gd.name AS rubric, grc.description, grl.definition, grl.score,
                                        grf.remark, grf.criterionid, rubm.username grader, stu.id AS userid,
                                        stu.idnumber AS idnumber, stu.firstname, stu.lastname, stu.username student,
...
if ($first === false) {
    ...
    redirect($url, get_string('norubricgrades', 'report_advancedgrades'), 5);
    exit;
}
...
$pos=report_advancedgrades_add_header($workbook, $sheet, $course->fullname, $cm->name, 'rubric', $first->rubric);
$datacol=$pos;
...
report_advancedgrades_add_data($sheet, $students, $gradinginfopos, 'rubric',$datacol);
